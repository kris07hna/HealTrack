    ...
    case 'GET':
      try {
        ...
        const avgMood = data.length > 0 
          ? (data.reduce((sum: number, entry: any) => sum + entry.mood_value, 0) / data.length).toFixed(1)
          : 0;
    ...
    const stats = {
      symptoms: {
        ...
        avgSeverity: symptomsResult.data?.length 
          ? (symptomsResult.data.reduce((sum: number, s: any) => sum + s.severity, 0) / symptomsResult.data.length).toFixed(1)
          : 0
      },
      mood: {
        entries: moodResult.data?.length || 0,
        average: moodResult.data?.length
          ? (moodResult.data.reduce((sum: number, m: any) => sum + m.mood_value, 0) / moodResult.data.length).toFixed(1)
          : 0
      },
      goals: {
        total: goalsResult.data?.length || 0,
        achieved: goalsResult.data?.filter((g: any) => g.achieved).length || 0,
        achievementRate: goalsResult.data?.length
          ? ((goalsResult.data.filter((g: any) => g.achieved).length / goalsResult.data.length) * 100).toFixed(1)
          : 0
      },
      meditation: {
        sessions: meditationResult.data?.length || 0,
        totalMinutes: meditationResult.data?.reduce((sum: number, m: any) => sum + (m.completed ? m.duration : 0), 0) || 0,
        completedSessions: meditationResult.data?.filter((m: any) => m.completed).length || 0
      }
    };
